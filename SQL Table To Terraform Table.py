...
#     sql = sql.upper()
    ...
    for command in ddl:
        ...
        for create_command in create_commands:
            ...
            # this code for find the column name 
            column_matches = re.findall(r'"([^"]+)"|([a-zA-Z_][\w\s/\-*^]*)\s+(?:AS\s+)?(?:(?:VARCHAR|CHAR|NUMBER|BINARY|STRING|TIMESTAMP_NTZ|TIMESTAMP_TZ|TIMESTAMP|TIME|ARRAY|VARIANT|OBJECT|TIMESTAMP_LTZ|GEOGRAPHY|GEOMETRY|BLOB|CLOB|TINYINT|DECIMAL)\([\d,]+\)|ARRAY|VARCHAR|TIMESTAMP_NTZ|TIMESTAMP_TZ|TIMESTAMP|BOOLEAN|TEXT|DATE|INT|INTEGER|FLOAT|VARIANT|OBJECT|TIMESTAMP_LTZ|GEOGRAPHY|GEOMETRY|BLOB|CLOB|TINYINT|DECIMAL)', sql.replace('\n', ' '))
            ...
            # this code for find the Not Null column name 
            ...
            # this code for find the Comment column name 
            ...
            # this code for find generated_always_as in column 
            ...
            # this code for find DEFAULT value in column 
            ...
            # this code for find Check value in column 
            pattern_check = r'(?:\"(.*?)\"|\b(\w+)\b)\s+.*?CHECK\s+\((.*?)\)\s*(?:(?:COMMENT|NOT\s+NULL|,\s*$))'
            check_matches = re.findall(pattern_check, sql)
            # ----------------------------------------------------------------------------------
            
            # this code for find the data type 
            
            data_type_matches = re.findall(r'("([^"]+)"|([\w\s/*&^?!-]+))\s+((?:VARCHAR|CHAR|NUMBER|BINARY|STRING|TIMESTAMP_NTZ|TIMESTAMP_TZ|TIMESTAMP|TIME|BOOLEAN|TEXT|DATE|INT|INTEGER|FLOAT|VARIANT|ARRAY|OBJECT|TIMESTAMP_LTZ|GEOGRAPHY|GEOMETRY|BLOB|CLOB|TINYINT|DECIMAL)(?:\([\d,]+\))?)', sql.replace('\n', ' '))
            ...
            # here put the  nullable value comment_value 
            for  col,j in zip(column_names,data_types_list):
                    code += f"\ncolumn {{\n"
                    code += f"\tname = \"{col}\"\n"
                    code += f"\ttype = \"{j}\"\n"
                    
                    # this code for generated_always_as Column
