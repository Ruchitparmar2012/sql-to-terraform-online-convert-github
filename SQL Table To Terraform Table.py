    ...
    for command in ddl:
        ...
        for create_command in create_commands:
            ...
            if table_comment_match:
                ...
                joind_table = "_".join(table)
                print(f"This table doesn't have table comments - {joind_table}")
...
for i, sql_contents in enumerate(sql_contents_list):
    ...
    for resource_name in resource_table_name_list:
        output_filename = os.path.join(output_folder, f"{resource_name}.tf")
        try:
            with open(output_filename, 'a') as tf_file:
                tf_file.write(main)
        except Exception as e:
