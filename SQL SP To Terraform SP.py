...
#         if command.startswith("CREATE "):
        ...
        for create_command in create_commands:
            ...
            statement_pattern = r"(AS '(?:[^']|''|';)*';)"

            # Use re.search() to find the first occurrence of the pattern in the SQL code
            match = re.search(statement_pattern, sql, re.DOTALL)
            statement_matches = match.group(0).strip("AS '").strip("';")
            # If there's a match, the extracted code will be in the first element of the 'matches' list
            if statement_matches:
                ...
                print("No code found between BEGIN and END.")
            ...
            execute_matches = re.findall(r"EXECUTE\s+AS\s+(\w+)", sql, re.IGNORECASE)
